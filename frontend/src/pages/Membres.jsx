...
function Membres() {
  ...
  return (
    <div className="pt-2">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="font-heading text-xl font-semibold text-gray-800 dark:text-dark-text">Membres</h2>
  ...
  className="flex items-center gap-2 h-10 px-4 rounded-button border border-black/10 dark:border-dark-border text-gray-600 dark:text-dark-text-muted text-sm font-medium hover:bg-black/5 dark:hover:bg-dark-surface transition"
>
  ...
  className="w-full h-10 pl-10 pr-3 rounded-button bg-white dark:bg-dark-surface border border-black/5 dark:border-dark-border text-sm text-gray-800 dark:text-dark-text placeholder:text-gray-400 dark:placeholder:text-dark-text-muted focus:outline-none focus:ring-2 focus:ring-secondary/30 transition"
/>
      </div>

      {/* Grille de cartes */}
      {loading ? (
        <div className="bg-white dark:bg-dark-surface rounded-card border border-black/5 dark:border-dark-border p-10 text-center text-sm text-gray-400 dark:text-dark-text-muted">
          Chargement...
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredMembres.map((membre) => (
            <div
            key={membre.id}
            className="bg-white dark:bg-dark-surface rounded-card border border-black/5 dark:border-dark-border p-5 relative hover:shadow-sm dark:hover:border-dark-text-muted/30 transition-colors"
          >
              {/* Menu contextuel */}
              {canManage && (
  <button
  ...
  >
    <MoreVertical size={17} strokeWidth={1.75} />
              ...
              {openMenuId === membre.id && (
                <div className="absolute right-4 top-11 bg-white dark:bg-dark-surface-2 rounded-button shadow-lg border border-black/5 dark:border-dark-border py-1 z-10 w-48">
                  <button
  onClick={() => handleResetPassword(membre.id)}
  className="flex items-center gap-2 w-full px-3 py-2 text-xs text-gray-600 dark:text-dark-text-muted hover:bg-black/5 dark:hover:bg-dark-surface transition-colors"
>
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <p className="font-heading font-semibold text-gray-800 dark:text-dark-text mt-3">
  {membre.prenom} {membre.nom}
                ...
                {membre.roleBureau ? (
                  ...
                  <span className="text-xs px-2.5 py-1 rounded-full bg-gray-100 dark:bg-dark-surface-2 text-gray-500 dark:text-dark-text-muted font-medium mt-1.5">
  Membre
</span>
                )}
              </div>
              ...
              {/* Infos */}
              <div className="mt-4 pt-4 border-t border-black/5 dark:border-dark-border space-y-2">
                {membre.filiere && (
